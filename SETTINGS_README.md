...
### วันทำการ (ตารางนัดหมาย)

กำหนดโหมดของแต่ละวันในสัปดาห์ (`work_day_monday` … `work_day_sunday`) ใช้ร่วมกันทุกช่องจอด มี 3 ค่า:

| โหมด | ตัวคูณของ `Service Bay.daily_capacity_hours` |
|---|---|
| เต็มวัน | 1 |
| ครึ่งวัน | 0.5 (ช่องจอด 8 ชม. เหลือ 4 ชม.) |
| หยุด | 0 (ปิดทำการ) |

ค่าเริ่มต้น จ.–ศ. เต็มวัน, เสาร์ ครึ่งวัน, อาทิตย์ หยุด — เติมลง DB โดย patch `set_working_day_defaults`
ค่าที่ยังไม่เคยบันทึก (NULL) หรือค่าที่ไม่รู้จัก ถือว่า **เต็มวัน** เพื่อไม่ให้ site ที่ยังไม่ได้ seed กลายเป็นปิดทั้งสัปดาห์

**ลำดับความสำคัญของชั่วโมงรับงานในวันหนึ่ง:**
`Bay Capacity Override` ที่ระบุช่องจอด > `Bay Capacity Override` ที่เว้นช่องจอดว่าง (มีผลทุกช่อง) > `daily_capacity_hours` คูณตัวคูณข้างบน
จึงเปิดวันหยุดประจำสัปดาห์ได้ด้วย override > 0 และปิดวันทำการได้ด้วย override = 0

> **ไม่มีการบล็อกการจอง** — จองเกินความจุ จองในวันหยุด หรือจองงานที่ยาวกว่าความจุต่อวัน ระบบขึ้นคำเตือน
> (และ confirm ก่อนบันทึกในหน้า desk) แต่บันทึกต่อได้เสมอ ส่วนที่เกินถือเป็นงาน OT ตามที่ตกลงกับผู้ใช้
